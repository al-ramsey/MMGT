...
\[x \xrightarrow{u} (Ff)(y)\xrightarrow{(Ff)(v)} (Ff \circ Fg)(z) \xrightarrow{(F^2_{f,g})_z} (F(g\circ f))(z),\]
...
For a map \(c \to d\) in \(\mathcal{C}\), we define a functor 
\begin{align*}
p^{-1}\{d\}&\to p^{-1}\{c\},
\end{align*}
which sends \(x \in p^{-1}\{d\}\) to a cartesian edge \(f : x' \to x\). It can be shown that these data assemble into a pseudofunctor \(\mathcal{C}^{\text{op}}\to \textbf{Cat}\).

I should dualise the above to make it not ridiculous, but I'm already many circles into op hell now, and it's also past 1am. 
