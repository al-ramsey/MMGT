...
We will not prove the theorems above (see \autocite{monoidalgrothendieck} for more details) but we will describe how to pass between the two \(2\)-categories.

Given a pseudofunctor \(\mathcal{C} \to \textbf{Cat}\), we want an opfibration \(X \to \mathcal{C}\). 

Given an opfibration \(X \to \mathcal{C}\), how do we get a pseudofunctor \(\mathcal{C} \to \textbf{Cat}\)? 
