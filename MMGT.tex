...
We dualise\footnote{The construction referenced builds a fibration \(X\to \mathcal{C}\) from a pseudofunctor \(\mathcal{C}^{\text{op}}\to \textbf{Cat}\); it is indeed possible to use this with \((\mathcal{C}^{\text{op}})^{\text{op}}\) to find a fibration \(\tilde X \to \mathcal{C}^{\text{op}}\) and hence an opfibration \(\tilde X^{\text{op}}\to \mathcal{C}\), but this is \textit{not} the same as the construction we give above. Welcome to op hell.} the construction in \autocite{2dimensional}.
